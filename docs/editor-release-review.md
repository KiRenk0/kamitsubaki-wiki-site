# 站内编辑 → GitHub PR：发布验收记录

历史验收日期：2026-09-10；最新进展见文末 2026-09-13 发布验证记录。前后端均在隔离工作树的 `codex/wiki-editor-pr` 分支。
...
用户已授权验收后直接上线。前端已合入最新 main a52f6d08 搜索修复，334 测试通过；后端 242 测试通过。Astro check 为 0 错误、0 警告、16 提示。正式配置构建 9,314 页面，14,762 资源通过体积审计，最大 22.38 MiB。Worker dry-run 通过。

Chrome 本地编辑器重新验证原文冲突核对、投稿、审核评论和修改要求、合并等待发布、部署失败和成功；这些审核事件为本地模拟。真实 GitHub App 已安装配置，PR #48 创建、修订、读取评论/状态、关闭成功，未合并，main 原文未变化。按用户决定，分支保护仅作为建议，不阻断预检。

等待创建当前账户 Pages 只读令牌的确认；表单已准备。真实账号页面、GitHub Webhook、Cron 和生产发布提交匹配仍需验证。当前没有生产部署，没有执行远端 D1 迁移。详细回滚版本和 D1 恢复点记录在后端 docs/editor-rollout-20260913.md；未导出用户数据。
