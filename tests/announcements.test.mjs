...
async function readAnnouncement(locale) {
  const source = await readFile(
    new URL(`../src/content/announcements/2026/2026-07-20-v1-2-0/${locale}.md`, import.meta.url),
    'utf8',
  );
...
test('V1.2.0 announcement is complete and pinned in every locale', async () => {
  const announcements = await Promise.all(locales.map(readAnnouncement));

  assert.deepEqual(announcements.map((announcement) => announcement.locale), locales);
  assert.equal(announcements.every((announcement) => announcement.translationKey === '2026-07-20-v1-2-0'), true);
  assert.equal(announcements.every((announcement) => announcement.pinned === true), true);
  assert.equal(announcements.every((announcement) => announcement.title.includes('V1.2.0')), true);
