...
test('theme preference defaults to system and offers localized light, dark, and system options', async () => {
  const [layout, nav, homeNav, languageSwitcher, homePage, script, styles] = await Promise.all([
  ...
  assert.match(languageSwitcher, /traditionalLinks = \['zh-hk', 'zh-tw'\]/);
  ...
  assert.match(styles, /\.site-nav__language-family:hover \.site-nav__language-menu/);
  assert.match(styles, /\.site-nav__language-family:focus-within \.site-nav__language-menu/);
