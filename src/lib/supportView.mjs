...
export function renderSupportData(data,copy,locale) {
  ...
  const channels=data.channels.map((item,i)=>{
    ...
    return `<article class="support-channel" data-available="${Boolean(href)}" aria-labelledby="channel-${e(item.id)}"><div class="support-channel__top"><span class="support-index" aria-hidden="true">${String(i+1).padStart(2,'0')}</span><span class="support-status"><span aria-hidden="true"></span>${e(href?copy.available:copy.unavailable)}</span></div><h3 id="channel-${e(item.id)}">${local(item.name)}</h3><p>${local(item.description)}</p><div class="support-channel__action">${href?`<a class="support-button" href="${href}" target="_blank" rel="noopener noreferrer" aria-label="${e(copy.visit)} · ${local(item.name)} (${e(copy.external)})">${e(copy.visit)}<span aria-hidden="true">↗</span></a><span class="support-note">${e(copy.external)}</span>`:`<span class="support-note">${e(copy.unavailableNote)}</span>`}</div></article>`;
  }).join('');
