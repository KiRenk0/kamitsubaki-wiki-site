...
let uploadedAvatar=null,rendered=null,loadedOwner=null,loading=false;
...
const message=(target,error)=>{const code=error?.message;$(target).textContent=code==='account_changed'?c.accountChanged:c.error;};
...
function render() {
  ...
  if(!state.viewer){uploadedAvatar=null;$('[data-avatar-file]').value='';loadedOwner=null;rendered=null;$('[data-account-sessions]').replaceChildren();$('[data-profile-form]').reset();return;}
  if(loadedOwner!==state.viewer.userId && !loading){
    loading=true;const owner=state.viewer.userId;
    loadAccount().then(()=>{if(state.viewer?.userId===owner){loadedOwner=owner;void sessions().catch(e=>message('[data-sessions-status]',e));}}).catch(e=>message('[data-account-action-status]',e)).finally(()=>{loading=false;if(state.viewer?.userId && state.viewer.userId!==owner)render();});
  }
  if(state.account && rendered!==state.account){
    $('[data-guest-preview-result]').hidden=true;$('[data-guest-items]').replaceChildren();
    rendered=state.account;const profile=state.account.profile,form=$('[data-profile-form]');
    form.elements.displayName.value=profile.display_name;form.elements.locale.value=profile.primary_locale;form.elements.avatar.value='keep';uploadedAvatar=null;$('[data-avatar-file]').value='';avatar();
...
if(root){
  ...
  $('[data-avatar-file]').addEventListener('change',async event=>{
    ...
    try{
      ...
      $('[data-profile-form]').elements.avatar.value='upload';avatar();$('[data-profile-status]').textContent='';
    }catch{uploadedAvatar=null;$('[data-profile-status]').textContent=c.imageError;}
  });
  $('[data-profile-form]').addEventListener('input',avatar);
  $('[data-profile-form]').addEventListener('submit',event=>{
    ...
    void action(form.querySelector('button'),'[data-profile-status]',async()=>{
      try {
        const account=await api('/api/account/profile',{method:'PATCH',body:{displayName:form.elements.displayName.value,locale:form.elements.locale.value,avatar:form.elements.avatar.value,...(form.elements.avatar.value==='upload'?{avatarData:uploadedAvatar}:{}),version:state.account.profile.profile_version}});
        state.account=account;render();$('[data-profile-status]').textContent=c.saved;await refreshAuth(true);
      } catch(e){if(e.message==='profile_conflict'){await loadAccount();$('[data-profile-status]').textContent=c.profileConflict;}else throw e;}
    });
  });
