---
locale: ja
translationKey: kaf_tomadoi_telepathy
title: "戸惑いテレパシー"
romanizedTitle: "Tomadoi Telepathy"
artist: 花譜
type: "EP"
description: "3.5D × docomoの展示テーマ曲を中心に、3種の公式リミックスとインストを収めたEP。"
releaseDate: "2020-06-10"
label: "KAMITSUBAKI RECORD"
catalogNumber: "ANTCD-46556"
trackCount: 5
duration: "18:31"
categoryTitle: "EP"
categorySubtitle: EPS
categoryOrder: 1
itemOrder: 15
image: /images/albums/kaf/tomadoi-telepathy.jpg
officialLinks:
  - label: "花譜オフィシャルサイト"
    href: https://kaf.kamitsubaki.jp/discography/20200610/110/
  - label: "Apple Music 日本"
    href: https://music.apple.com/jp/album/%E6%88%B8%E6%83%91%E3%81%84%E3%83%86%E3%83%AC%E3%83%91%E3%82%B7%E3%83%BC-ep/1688524197?uo=4
tracks:
  -
    number: "1"
    title: "戸惑いテレパシー"
    duration: "03:48"
    songId: "kaf/originals/戸惑いテレパシー-confused-telepathy"
  -
    number: "2"
    title: "戸惑いテレパシー(柊キライ Remix)"
    duration: "03:07"
    songId: "kaf/remixes/戸惑いテレパシー(柊キライ-Remix)-confused-telepathy-kirai-hiiragi-remix"
  -
    number: "3"
    title: "戸惑いテレパシー(Orangestar Remix)"
    duration: "03:50"
    songId: "kaf/remixes/戸惑いテレパシー(Orangestar-Remix)-confused-telepathy-orangestar-remix"
  -
    number: "4"
    title: "戸惑いテレパシー(羽生まゐご Remix)"
    duration: "03:58"
    songId: "kaf/remixes/戸惑いテレパシー(羽生まゐご-Remix)-confused-telepathy-maigo-hanyuu-remix"
  -
    number: "5"
    title: "戸惑いテレパシー(Instrumental)"
    duration: "03:48"
    songId: "kaf/instrumentals/戸惑いテレパシー(Instrumental)-confused-telepathy-instrumental"
---

## 作品概要

3.5D × docomoの展示テーマ曲を中心に、3種の公式リミックスとインストを収めたEP。

## リリースと仕様

- 発売日・曲順・再生時間は公式作品ページとApple Music日本版を照合しています。
- 公式の収録順で校合済みです。MCを除く全トラックから、個別の楽曲またはバージョン記事へ移動できます。

## トラック情報

全5トラック。上部のトラック表にはディスク番号、曲順、再生時間、共演者、サイト内楽曲リンクを収録しています。

## 出典

- [花譜オフィシャルサイト](https://kaf.kamitsubaki.jp/discography/20200610/110/)
- [Apple Music](https://music.apple.com/jp/album/%E6%88%B8%E6%83%91%E3%81%84%E3%83%86%E3%83%AC%E3%83%91%E3%82%B7%E3%83%BC-ep/1688524197?uo=4)
- [VGMdb 花譜ディスコグラフィー](https://vgmdb.net/artist/34690)
