---
locale: "ja"
translationKey: "kafu"
code: "11"
romanizedName: "KAFU"
categoryOrder: 3
itemOrder: 1
meta: "RELEASE: 2021.07.07"
debutDate: "2021-07-07"
profileTagline: "花譜の歌声をもとに生まれた音楽的同位体 CeVIO AI / Synthesizer V ライブラリ。"
designCredits:
  - "キャラクターデザイン：PALOW."
affiliations:
  - "KAMITSUBAKI STUDIO"
  - "PHENOMENON RECORD"
officialLinks:
  - label: "音楽的同位体 可不 公式サイト"
    href: "https://kafu.kamitsubaki.jp/"
featuredEntries:
  - label: "花譜 / KAF"
    href: "/ja/artists/vwp/kaf"
    kind: "artist"
theme:
  name: "KAFU Yellow"
  accentColor: "#f7ca00"
  mutedColor: "#a38200"
  surfaceColor: "#14130d"
  highlightColor: "#fffde8"
  palette:
    - label: "可不イエロー"
      value: "#F7CA00"
    - label: "ゴールド"
      value: "#A38200"
    - label: "ウォームホワイト"
      value: "#FFFDE8"
    - label: "ナイト"
      value: "#14130D"
statusLabel: "STATUS"
status: "ACTIVE"
image: "https://placehold.co/1200x800/14130d/f7ca00?text=KAFU"
name: "可不"
categoryTitle: "音楽的同位体"
categorySubtitle: "MUSICAL ISOTOPES"
seo:
  keywords:
    - "可不"
    - "KAFU"
    - "音楽的同位体"
    - "CeVIO AI"
    - "フォニイ"
---

## 概要

{{ruby::可不::かふ::kafu}} は、[KAMITSUBAKI STUDIO](/ja/projects/labels/kamitsubaki-studio) が展開する「音楽的同位体」シリーズの第1弾。バーチャルシンガー **{{ruby::花譜::かふ::kaf}}** の歌声をもとに開発された合成音声ライブラリ。

2021年7月7日に CeVIO AI 版が発売。ツミキによる《フォニイ》をはじめ、数々のメガヒット曲を生み出し、ボカロシーンを代表する音声ライブラリとなった。

<div class="wiki-fact-grid">
  <div class="wiki-fact-card"><p class="wiki-fact-card__label">VOICE PROVIDER</p><p class="wiki-fact-card__value">[花譜](/ja/artists/vwp/kaf) / KAF</p></div>
  <div class="wiki-fact-card"><p class="wiki-fact-card__label">SINGING ENGINE</p><p class="wiki-fact-card__value">CeVIO AI、のちに Synthesizer V AI も展開</p></div>
  <div class="wiki-fact-card"><p class="wiki-fact-card__label">CHARACTER DESIGN</p><p class="wiki-fact-card__value">[PALOW.](/ja/artists/creators/palow)</p></div>
  <div class="wiki-fact-card"><p class="wiki-fact-card__label">RELEASE</p><p class="wiki-fact-card__value">2021年7月7日</p></div>
</div>

## デザインと声の位置づけ

PALOW.による白と黄色を軸にしたデザイン、環状の瞳、フード付きの衣装、幾何学的なディテールは、花譜とのつながりを保ちながら、可不を独立した音声キャラクターとして成立させている。

## 歩み

| 時期 | 出来事 | 内容 |
| --- | --- | --- |
| 2020年10月 | 声質アンケート | 3種類の試作歌唱タイプを公開し、Type B を製品の基礎に決定。 |
| 2020年11月～12月 | 先行カバー | 《[雛鳥](/ja/songs/kaf/originals/hinadori)》《[心臓と絡繰](/ja/songs/kaf/originals/shinzou-to-karakuri)》《過去を喰らう》などを公開。 |
| 2021年5月～7月 | 委嘱曲と発売 | 多数のクリエイターが参加する企画を経て、7月7日に正式発売。 |
| その後 | エンジン拡張 | CeVIO AI 版に加え、Synthesizer V AI 版も展開。 |

## 委嘱曲インデックス

{{details::立ち上げ期の曲目を表示}}

| 日付 | 楽曲 | クリエイター |
| --- | --- | --- |
| 2021年1月9日 | 《ホログラム》 | 一二三 |
| 2021年2月20日 | 《[キュートなカノジョ](/ja/songs/harusaruhi/covers/キュートなカノジョ-cute-girlfriend)》 | syudou |
| 2021年3月7日 | 《アイスクリーム》 | [Guiano](/ja/artists/creators/guiano) |
| 2021年4月10日 | 《レトロポリス》 | R Sound Design |
| 2021年5月1日 | 《花となれ》 | 雄之助 |
| 2021年5月8日 | 《水面下》 | 大沼パセリ |
| 2021年6月5日 | 《フォニイ》 | ツミキ |
| 2021年7月17日 | 《[キャットラビング](/ja/songs/kashiimoimi/originals/kyattorabingu)》 | [香椎モイミ](/ja/artists/creators/kashiimoimi) |

{{/details}}

これらは初期の公式委嘱曲をたどるための索引であり、可不の全作品目録ではない。《フォニイ》《マーシャル・マキシマイザー》《キュートなカノジョ》などを含む、クリエイターによる継続的な作品群こそが可不の活動領域を形づくっている。

## プロジェクト内のつながり

可不は[音楽的同位体](/ja/artists/solo/musical-isotope)プロジェクトの出発点であり、[星界 / SEKAI](/ja/artists/isotopes/sekai)、[裏命 / RIME](/ja/artists/isotopes/rime)、[狐子 / COKO](/ja/artists/isotopes/coko)、[羽累 / HARU](/ja/artists/isotopes/haru)へと続くクリエイター向けの歌声キャラクター群の基礎になった。
## 外部リンク

- [音楽的同位体 可不 公式サイト](https://kafu.kamitsubaki.jp/)
