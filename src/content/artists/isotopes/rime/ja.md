---
locale: "ja"
translationKey: "rime"
code: "13"
romanizedName: "RIME"
categoryOrder: 3
itemOrder: 3
meta: "ORIGIN: RIM"
debutDate: "2022-10-25"
profileTagline: "理芽の歌声をもとに開発された CeVIO AI 音楽的同位体。透明感と情熱的なエモーションを併せ持つ合成音声をクリエイターに提供。"
designCredits:
  - "キャラクターデザイン：PALOW."
affiliations:
  - "KAMITSUBAKI STUDIO"
  - "音楽的同位体プロジェクト"
officialLinks:
  - label: "音楽的同位体公式サイト"
    href: "https://musical-isotope.kamitsubaki.jp/"
  - label: "X (Twitter)"
    href: "https://twitter.com/musicalisotope"
  - label: "YouTube"
    href: "https://www.youtube.com/channel/UCm_1n9F3A6pz-Ms2NP93tBA"
featuredEntries:
  - label: "理芽 / RIM"
    href: "/ja/artists/vwp/rim"
    kind: "artist"
  - label: "可不 / KAFU"
    href: "/ja/artists/isotopes/kafu"
    kind: "artist"
  - label: "星界 / SEKAI"
    href: "/ja/artists/isotopes/sekai"
    kind: "artist"
  - label: "狐子 / COKO"
    href: "/ja/artists/isotopes/coko"
    kind: "artist"
  - label: "羽累 / HARU"
    href: "/ja/artists/isotopes/haru"
    kind: "artist"
theme:
  name: "RIME Resonance"
  accentColor: "#e51500"
  mutedColor: "#b91c1c"
  surfaceColor: "#1a0f0f"
  highlightColor: "#fff5f5"
  palette:
    - label: "裏命赤"
      value: "#E51500"
    - label: "共鳴赤"
      value: "#B91C1C"
    - label: "深緋"
      value: "#1A0F0F"
    - label: "ソフトホワイト"
      value: "#FFF5F5"
statusLabel: "TYPE"
status: "ACTIVE"
image: "https://placehold.co/1200x800/1a0f0f/e51500?text=RIME"
name: "裏命"
categoryTitle: "音楽的同位体"
categorySubtitle: "MUSICAL ISOTOPE"
seo:
  keywords:
    - "裏命"
    - "RIME"
    - "CeVIO AI"
    - "音楽的同位体"
    - "KAMITSUBAKI STUDIO"
---

## 概要

裏命（RIME）は、KAMITSUBAKI STUDIOが展開する「音楽的同位体」シリーズの第3弾としてリリースされた音声合成ソフトウェアです。バーチャルシンガー理芽（RIM）の歌声データをベースに、AI技術を用いて本人の声質をリアルに再現しています。

CeVIO AIプラットフォーム上で動作し、名前は「RIM + ECHO（理芽と響き）」というコンセプトに由来しています。理芽の歌声が持つ透明感と怪しげなニュアンスを引き継ぎつつ、強い感情表現やエモーショナルな表現を得意としています。

## キャラクターと創作における位置づけ

キャラクターデザインはPALOW.が担当しており、白髪に濃いブルーのメッシュ、八芒星のヘアアクセサリーが特徴的です。音声面ではリアルなブレス音や繊細な表現調整に対応し、アップテンポなデジタル・ロック調からスローバラードまでエモーショナルに歌いこなします。

音楽的同位体プロジェクトの理念に基づき、裏命はクリエイターの「創作のパートナー」として定義されています。UGC（ユーザー生成コンテンツ）の領域において、ボカロPや音声合成ユーザーによって多くの二次創作・オリジナル楽曲が公開されています。

## 歩み

### 2022年

- **4月16日**：V.W.Pの1st ONE-MAN LIVE「現象」のエンドロールにて、音楽的同位体「裏命（RIME）」のティザー映像が公開され、その存在が初めて発表される。
- **7月7日**：先行予約受付が開始され、YouTubeにて初のカバー曲『食虫植物』（原曲：花譜）のMVが公開される。
- **7月14日**：R Sound Designによる初のオリジナル曲『マンダリン』が公開される。
- **9月30日**：理芽と裏命によるデュエット曲『NEUROMANCE feat. 裏命 (from Singularity Live)』が公開される。
- **10月25日**：歌声合成ソフトウェア「CeVIO AI 音楽的同位体 裏命（RIME）」が正式発売。PALOW.による公式キャラクターデザインが公開される。
- **12月5日**：イラスト・楽曲・MVコンテストが開催される。

### 2023年

- **4月4日**：39曲のオリジナル楽曲を収録した裏命の1stコンピレーションアルバム『パラノーマル』のリリースが発表される。
- **5月10日**：コンピレーションアルバム第1弾『残響のパラノーマル』が先行配信開始。
- **5月24日**：コンピレーションアルバム第2弾『玉響のパラノーマル』が先行配信開始。
- **6月7日**：コンピレーションアルバム第3弾『交響のパラノーマル』が配信開始。あわせてCDボックスが正式発売。
- **9月13日**：コンピレーションアルバム第4弾『空想のパラノーマル』が配信開始。
- **11月22日**：楽曲コンテストで大賞を受賞した、はるな。による『花降るる』のMVが公開される。
- **12月16日**：入力文字読み上げソフト「音楽的同位体 裏命 TALK EXTENSION collaboration with VOICEPEAK」の制作決定が発表され、12月18日より予約受付が開始。

### 2024年

- **2月26日**：「音楽的同位体 裏命 TALK EXTENSION collaboration with VOICEPEAK」が正式発売。
- **7月7日**：音楽的同位体プロジェクト3周年を記念したバーチャルミニライブ「V.I.P 1st MINI LIVE ETHEREAL WORLD」に、グループ「V.I.P」のメンバーとして出演。
- **8月8日**：パシフィコ横浜にて開催された「KAMITSUBAKI FES '24 THE DAY THE EARTH STOOD STILL」に「V.I.P」として出演。

### 2025年

- **11月2日**：Kanadevia Hallにて開催された「KAMITSUBAKI FES '25 OUR ONE-DAY WAR」に「V.I.P」として出演。

## 代表曲と関連項目

裏命を用いた代表的な楽曲には以下のようなものがあります。

- 『食虫植物』（裏命 カバーバージョン）
- 『甘美な無法』（裏命 カバーバージョン）
- UGCクリエイターによる数多くのオリジナル曲（R Sound Designによる『マンダリン』、ピコンによる『傷心中』など）

おすすめの関連項目：

- [理芽 / RIM](/ja/artists/vwp/rim)
- [可不 / KAFU](/ja/artists/isotopes/kafu)
- [星界 / SEKAI](/ja/artists/isotopes/sekai)

## 基本資料とキャラクター設定

裏命は理芽の歌声データをもとにした CeVIO AI 歌声合成ソフトウェアおよびキャラクターである。理芽の透明感と妖しさを受け継ぎながら、独立した創作上の存在として展開している。

| 項目 | 内容 |
| --- | --- |
| 声の提供元 | 理芽 |
| プラットフォーム | CeVIO AI、VOICEPEAK TALK EXTENSION |
| キャラクターデザイン | PALOW. |
| 主な企画 | 『Paranormal』、裏命楽曲コンテスト |

## 代表曲とクリエイター・エコシステム

「食虫植物」は理芽の作品と裏命をつなぐ公式カバーであり、「Mandarin」は裏命が独立した合成歌声として機能することを示す。『Paranormal』合輯はUGC作品を正式なリリース史にまとめ、楽曲コンテストはオリジナル曲やMVへ創作範囲を広げた。

理芽の歌唱、裏命の歌唱、理芽と裏命の合唱は別々に記録する。TALK EXTENSIONによって、歌唱だけでなく音声表現にも用途が広がっている。

## 公式音楽とコミュニティ作品

裏命の歩みは、個人歌手のディスコグラフィーとは異なり、公式デモ、コンテスト、クリエイター合輯が主要な節目となる。『Paranormal』は音楽作品であると同時に、創作者コミュニティの記録でもある。

## 関連プロジェクト・設定

裏命は、KAMITSUBAKI STUDIOが提唱する「音楽的同位体プロジェクト」の中核製品です。バーチャルシンガーとしての理芽自身の枠組みを超え、二次創作や創作エコシステムにおいて人々の手によって無限に再構成される、声という共有の「クリエイティブ素材」としての展開を体現しています。

## 外部リンク

- [音楽的同位体 公式サイト](https://musical-isotope.kamitsubaki.jp/)
- [音楽的同位体 公式 X (Twitter)](https://twitter.com/musicalisotope)
- [音楽的同位体 公式 YouTubeチャンネル](https://www.youtube.com/channel/UCm_1n9F3A6pz-Ms2NP93tBA)
