---
locale: "en"
translationKey: "teresa"
code: "10"
romanizedName: "teresaAI"
categoryOrder: 2
itemOrder: 4
meta: "SPLIT: 2023.03"
debutDate: "2020-05-29"
profileTagline: "A 3DCG virtual singer born in Tokyo and raised in Los Angeles, known for bilingual pop and a landmark split into voice-synthesis software and a human artist identity."
designCredits:
  - "3DCG character production: undisclosed"
affiliations:
  - "SINSEKAI RECORD"
officialLinks:
  - label: "YouTube"
    href: "https://www.youtube.com/channel/UCqG0skCM-dky7KGazl9bLqA/"
  - label: "Twitter"
    href: "https://twitter.com/virtual_teresa"
  - label: "Instagram"
    href: "https://www.instagram.com/virtual_teresa/"
  - label: "Weibo"
    href: "https://www.weibo.com/u/7454971009"
  - label: "bilibili"
    href: "https://space.bilibili.com/586849155"
featuredEntries: []
theme:
  name: "teresa Dawn"
  accentColor: "#6ea8fe"
  mutedColor: "#3a5ba0"
  surfaceColor: "#0e1525"
  highlightColor: "#eef4ff"
  palette:
    - label: "teresa Blue"
      value: "#6EA8FE"
    - label: "Dawn Navy"
      value: "#3A5BA0"
    - label: "Deep Surface"
      value: "#0E1525"
    - label: "Soft Light"
      value: "#EEF4FF"
statusLabel: "STATUS"
status: "ACTIVE"
image: "https://placehold.co/1200x800/0e1525/6ea8fe?text=teresaAI"
name: "teresaAI"
categoryTitle: "Solo Artists"
categorySubtitle: "SINSEKAI RECORD"
seo:
  keywords:
    - "teresaAI"
    - "te'resa"
    - "SINSEKAI RECORD"
    - "virtual singer"
---

## Overview

teresaAI is a 3DCG virtual singer who first appeared under the name te'resa, born in Tokyo and raised in Los Angeles. She gained attention through a bilingual approach to pop music and notable mainstream crossover moments, including appearances in Intel television commercials broadcast in China and a selection for NYLON'S NEXT 2021.

In March 2023 the project underwent a defining transformation, splitting into two distinct entities: teresaAI, a voice-synthesis software product preserving the original vocal identity, and ELENA (also styled ELЁNA), the real artist continuing a human-centered creative path. This dual outcome reflects broader questions about the boundary between virtual performance and authentic vocal artistry.

## Role and Creative Position

teresaAI's work bridges Japanese and English-language pop, shaped by a trans-Pacific upbringing. Her catalog ranges from bright, accessible singles like "I Wanna Be Your Girl!" to more mature, atmospheric releases such as the EP *predawn*. Collaborations with other SINSEKAI ecosystem artists, including a notable "IMAGINARY WORLD" live performance with Rim in June 2022, reinforced her position within the label's network.

The 2023 split positioned teresaAI not just as a historical artist project but as a product: a voice-synthesis engine carrying the original virtual singer's timbre into creator-driven production.

## Activity History

- Debuted as te'resa, a 3DCG virtual singer active across YouTube, Bilibili, and Weibo.
- Appeared in Intel TV commercials in China, gaining mainstream visibility.
- Selected for NYLON'S NEXT 2021.
- **June 25, 2022**: Performed in her first two-man live, "IMAGINARY WORLD," with Rim.
- **February 22, 2023**: Participated in "αU research × KAMITSUBAKI STUDIO presents prompt αU".
- **March 2023**: Split into voice-synthesis software "teresaAI" and human artist "ELЁNA".

## Representative Works and Related Entries

- EPs: *Ace* (1st EP), *predawn* (2nd EP)
- Singles: "I Wanna Be Your Girl!", "RED", "Don't slow me down"

## Related Projects / Setting

teresaAI operates within the SINSEKAI RECORD ecosystem, a label that houses multiple virtual and semi-virtual artists. The project's evolution from te'resa into a software-and-artist pair mirrors a wider trend in the virtual singer space, where voice identity increasingly exists independently of any single performer or avatar.

## References

- teresaAI YouTube channel: <https://www.youtube.com/channel/UCqG0skCM-dky7KGazl9bLqA/>
- teresaAI Twitter: <https://twitter.com/virtual_teresa>

## External Links

- [YouTube](https://www.youtube.com/channel/UCqG0skCM-dky7KGazl9bLqA/)
- [Twitter](https://twitter.com/virtual_teresa)
- [Instagram](https://www.instagram.com/virtual_teresa/)
