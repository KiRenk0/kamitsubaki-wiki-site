---
locale: "ja"
translationKey: "anmc"
code: "15"
romanizedName: "ANMC"
categoryOrder: 2
itemOrder: 9
meta: "PROJECT: ANMC"
profileTagline: "ゲームクリエイター岡秀英による「indie game × music」プロジェクト。シナリオの段階からゲーム・音楽・MVを同時制作する。"
designCredits: []
affiliations:
  - "KAMITSUBAKI STUDIO"
  - "ANARCHIC RECORD"
officialLinks:
  - label: "Lit.Link"
    href: "https://lit.link/anmc"
  - label: "X (Twitter)"
    href: "https://x.com/ANMCkamitsubaki"
  - label: "YouTube"
    href: "https://youtube.com/@ANMC_KAMITSUBAKI"
  - label: "TikTok"
    href: "https://www.tiktok.com/@anmc_kamitsubaki"
  - label: "Steam"
    href: "https://store.steampowered.com/app/2951340"
featuredEntries:
  - label: "Empty old City"
    href: "/ja/artists/solo/empty-old-city"
    kind: "artist"
  - label: "Kazuhide Oka"
    href: "/ja/artists/creators/kazuhide-oka"
    kind: "artist"
theme:
  name: "ANMC Indie Game"
  accentColor: "#7ecbf0"
  mutedColor: "#4a90c4"
  surfaceColor: "#0e1a24"
  highlightColor: "#dff0ff"
  palette:
    - label: "スカイブルー"
      value: "#7ECBF0"
    - label: "ディープブルー"
      value: "#4A90C4"
    - label: "ダークナイト"
      value: "#0E1A24"
    - label: "ソフトホワイト"
      value: "#DFF0FF"
statusLabel: "STATUS"
status: "ACTIVE"
image: "/images/artists/anmc.png"
name: "ANMC"
categoryTitle: "ソロ / ユニット"
categorySubtitle: "SOLO ARTISTS & UNITS"
seo:
  keywords:
    - "ANMC"
    - "anomachi"
    - "KAMITSUBAKI STUDIO"
    - "あの街"
---

## 概要

ANMC（あのまち）は、ゲームクリエイターの岡秀英が手がける「indie game × music」プロジェクトである。2022 年に Steam でリリースされた『ナツノカナタ』で注目を集め、[KAMITSUBAKI STUDIO](/ja/projects/labels/kamitsubaki-studio) の ANARCHIC RECORD エコシステムおよび KAMITSUBAKI CREATION の枠組みで活動している。

コンセプトは「音楽がゲームの一片になる」。シナリオの段階からゲーム・音楽・MV を同時制作することで、ゲーム世界「あの街」にクリエイターが集まるという独自の創作モデルを構築している。

## 概念と制作手法

ANMC の特徴は、ゲームと音楽を切り離さずに一つの作品として成立させることにある。楽曲はゲーム内のシナリオから生まれ、MV はゲーム世界の映像として構成される。すべてがシナリオという共通の起点から派生するため、メディア横断的な一体感が生まれる。

Steam では『ナツノカナタ』（App ID: 2951340）と第 2 作（App ID: 3337210）が配信中。2026年6月12日には新作ゲーム『プラトニカ・スペース』のデモが公開、同年 3 月 20 日には『がーるずメイドぷりん』の 1 周年を迎えた。

## ディスコグラフィ

- 6th シングル：「また旅はネコミミと」feat. kahoca（[Empty old City](/ja/artists/solo/empty-old-city)）、一ノ瀬陽鞠
- 5th シングル：「ふたりだけの物語」
- 4th シングル：「moonless」
- 3rd シングル：「月の匂い」feat. WaMi
- 2nd シングル：「Outline Seaside」feat. kahoca

## 参考資料

- KAMITSUBAKI STUDIO 公式：<https://kamitsubaki.jp>
- Steam：ナツノカナタ：<https://store.steampowered.com/app/2951340>

## 外部リンク

- [Lit.Link](https://lit.link/anmc)
- [X (Twitter)](https://x.com/ANMCkamitsubaki)
- [YouTube](https://youtube.com/@ANMC_KAMITSUBAKI)
- [TikTok](https://www.tiktok.com/@anmc_kamitsubaki)
- [Steam](https://store.steampowered.com/app/2951340)
