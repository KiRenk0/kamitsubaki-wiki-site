...
faqs:
  - question: 没有 GitHub 账号，现在能做什么？
    answer: 没有 GitHub 账号也可以使用 Google 登录 Wiki，通过站内编辑器提交已有词条的修改。只有直接走 GitHub 流程时才需要 GitHub 账号；未登录也可以编辑并下载本地草稿。
  - question: 草稿保存了，为什么网站还没变？
    answer: 保存草稿不会发布内容。站内流程还需要提交审核；GitHub 流程还需要创建 PR。两种方式都要经过审阅、合并与成功部署。浏览器草稿留在本机，云端草稿需在投稿面板手动保存。
